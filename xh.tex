...
(if (contains (defs) destructuring-bind)
  ''
...
# aliasing and array slicing.
(defn* destructuring-bind
  [template value bindings]
  (let [t (realtype $template)]
    (cond (== literal $t)
          (assoc $bindings $template $value)

          (== array $t)
          (reduce (fn [bs i]
                    (let [t (get $template $i)])))))) \end{xhcode}

  At this point one of the main problems with xh corescript is that it
  evaluates depth-first. This prevents us from passing a closure to itself, for
  example using a {\tt reduce} loop. We need to get around this problem by
  creating a better interpreter that uses a priority queue instead of a stack.
