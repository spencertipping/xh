...
\chapter{parser}\label{chp:base-parser}
  We define the real parser in \Ref{chapter}{chp:self-hosted-parser}, but
  because it's written in xh we need this one to parse that module. This one
  can be arbitrarily slow, since it's just used to bootstrap the image. It
  won't appear in distributable images of xh.

\lstset{caption={src/baseparser.pl},name={src/baseparser.pl}}\begin{perlcode}
BEGIN {xh::defmodule('xh::baseparser.pl', <<'_')}
# TODO: nail down the syntax and write a perl parser for it
_ \end{perlcode}

\part{standard library}\label{part:standard-library}
\chapter{core functions}\label{chp:core-functions}
  We have two functions to work with right now:~\verb|def| and \verb|amb|.
  Everything else is built on these and by equivalence with quoted constructs.

\lstset{caption={src/core.xh},name={src/core.xh}}\begin{xhcode}
-- include src/core/conditionals.xh
-- include src/core/iteration.xh \end{xhcode}

\section{conditionals}\label{sec:conditionals}
    xh doesn't have booleans, so we need to define how those work:

\lstset{caption={src/core/conditionals.xh},name={src/core/conditionals.xh}}\begin{xhcode}
def (true  $x $y) $x
def (false $x $y) $y
def (->bool $x)  true
def (->bool '')  false
...
def (if $cond $x $y)      ((->bool $cond) $x $y)
def (when $cond @$body)   (if $cond (do @$body) nil)
def (unless $cond @$body) (if $cond nil (do @$body))

def (cond $c $e) (if $c $e $undefined)
def (cond $c $e $default) (if $c $e $default)
def (cond $c1 $e1 $c2 $e2 @$rest) \
    (if $c1 $e1 (cond $c2 $e2 @$rest)) \end{xhcode}

\section{iteration}\label{sec:iteration}
\lstset{caption={src/core/iteration.xh},name={src/core/iteration.xh}}\begin{xhcode}
# TODO: more of these, but figure out functions vs abstract expressions
def (while $cond @$body) \
    (when $cond
      (do @$body)
      (while $cond @$body))
...
\section{literals}\label{sec:literals}
    This is where we encode all of xh's backslash-escaping logic and
    interpolation constructs.

\lstset{caption={src/parser/literals.xh},name={src/parser/literals.xh}}\begin{xhcode}
# TODO: interpolation; this is seriously nontrivial and I'm probably
# doing it wrong.
defn (xh-sstring $contents)  "'$(re-replace $contents
                                  "\\" "\\\\" "'"  "\\'")'"
defn (xh-dstring $contents) "\"$(re-replace $contents
                                  "\\" "\\\\" "\"" "\\\""
                                  "\n" '\n'   "\t" '\t')\"" \end{xhcode}
