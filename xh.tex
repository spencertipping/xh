...
\item{}
  ...
  and quotation level. These won't block when expanding unrealized lazy
  values.
\item{}
  \verb|!| distributes across \verb|@|:~\verb|!@$| and \verb|!@()|
  locally preserve list structure and quotation level, varying atom
  count. These forms also won't block when expanding unrealized lazy
  values.
\end{itemize}
...
\section{quoted form representation}\label{sec:quoted-representation}
    Like in Lisp, you can quote quoted forms to see how they are represented:

\begin{verbatim}
def form $x
echo $form                # will block until you def x
echo !$form               # -> (unquote x)
echo !!$form              # -> (unquote (list unquote x))
echo !@$form              # -> (unquote-splice x)
echo !!@$form             # -> (unquote (list unquote-splice x))
echo $!@$form             # will block until you def x
\end{verbatim}

    xh's prefix operators are generally composable, but it's worth keeping in
    mind that things like \verb|@| aren't well-defined by themselves; \verb|@$|
    means {\tt unquote-splice}.\footnote{{\bf TODO:} think this through
    carefully. It's starting to seem like {\tt unquote-splice} is some horrible
    hack that needs to be redesigned, though I do hate to push this
    functionality into some kind of {\tt apply} function.}
