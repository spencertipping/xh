...
\chapter{Evaluation semantics}\label{chp:evaluation-semantics}
  xh evaluation semantics can be defined in terms of parse trees. While we're
  at it, we also define the useful {\tt unquote} (analogous to Clojure's {\tt
  read-string}) and {\tt quote} (analogous to Clojure's {\tt pr-str}).

\lstset{caption={modules/eval.xh},name={modules/eval.xh}}\begin{xhcode}
(def (unquote $s) (grammar-apply $xh-parser $s)
     (quote   $s) (grammar-parse $xh-parser $s)) \end{xhcode}
