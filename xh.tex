...
\chapter{xh-script semantics}\label{chp:xhs-semantics}
  xh-script source is written as a series of quoted expressions, each of which
  will be unquoted to evaluate it. xh does a bit of preprocessing at the
  toplevel so you don't have to wrap each expression in parentheses, but
  semantically it works like Lisp. For example:
...
\end{verbatim}
  ...
  xh reads the whole file\footnote{Or incrementally; it doesn't matter.} and
  preprocesses it into these forms:
...
\end{verbatim}
  ...
  Now each form is unquoted, which causes it to be evaluated. Unquoting a
  \verb|()| form involves calling a function:
...
\end{verbatim}
  ...
  Once \verb|(def foo bar)| runs successfully, xh can unblock the evaluation of
  \verb|$foo|, and the evaluator continues:

\begin{verbatim}
(echo bar)
\end{verbatim}

\section{lazy values and quotation}\label{sec:lazy-values}
    Some operations won't complete immediately, for example reads from a file.
    Any normal expansions (i.e.~\verb|$| and \verb|()|) will block until the
    results become available, but you can view in-progress lazy values by
    asking for a quoted form of the value:
...
\end{verbatim}
    ...
    The rationale for this behavior is that any operation can be described
    rather than executed; if you refer to an operation this way, you have
    quoted it (much as Lisp supports quoted s-expressions). xh takes this idea
    one step further and represents any unknown or unrealized value as an
    opaque quantity, but one with a well-defined quoted form; this quoted form
    is first-class, so you can work with it without blocking.

    Another way to think of it is that \verb|!| tells you how much xh knows
    about a value. For example, suppose you concatenate two files:
...
\section{unquoting}\label{sec:unquoting}
    \verb|$| is a right-associative prefix operator, as are \verb|!|~and
    \verb|@|. Technically they are reader macros with the following expansions:
...
\end{verbatim}
    ...
    \verb|@| is slightly magical. Just as \verb|$| is a somewhat arbitrary way
    to dereference things, \verb|@$| and \verb|@()| are arbitrary syntactic
    forms that indicate list splicing.
...
\part{standard library}\label{part:standard-library}
\chapter{self-hosted parser}\label{chp:self-hosted-parser}
  Right now the Perl version of xh knows how to parse xh-script, but xh itself
  does not. We need to define the \verb|quote| function so that xh can generate
  parsers in other languages.

\lstset{caption={src/parser.xh},name={src/parser.xh}}\begin{xhcode}
-- include src/parser/syntax.xh
-- include src/parser/literals.xh
-- include src/parser/containers.xh \end{xhcode}

\section{base syntax}\label{sec:base-syntax}
\lstset{caption={src/parser/syntax.xh},name={src/parser/syntax.xh}}\begin{xhcode}
defn (repeat $form) (amb $form "$form$(repeat $form)")
defn (xh-comment $content) "#$(re-encode '[^\n]' $content)"
defn (xh-whitespace $ws)   (re-encode '\s' $ws)
defn (xh-ignored)          (repeat (amb (xh-comment $_)
                                        (xh-whitespace $_))) \end{xhcode}

\section{literals}\label{sec:literals}
    This is where we encode all of xh's backslash-escaping logic.

\lstset{caption={src/parser/literals.xh},name={src/parser/literals.xh}}\begin{xhcode}
 \end{xhcode}

\chapter{xh semantics}\label{chp:xh-semantics}
  All of xh's semantics are defined in terms of equations over quoted values.
  This chapter defines core observables of quoted data structures.

\lstset{caption={src/structures.xh},name={src/structures.xh}}\begin{xhcode}
-- include src/structures/vector.xh \end{xhcode}

\lstset{caption={src/structures/vector.xh},name={src/structures/vector.xh}}\begin{xhcode}
 \end{xhcode}
