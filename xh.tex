...
sub xh::corescript::fn::str {
  ...
  '(' . join(' ', $concrete ? 'fn' : 'macro',
                  array->new(map var->new($_), @$formals)->str,
                  $body->str) . ')';
}
...
defglobal if => sub {
  ...
  return list->new(literal->new('if'), $cond, $then, $else)
...
defnglobal 'def!' => sub {
  ...
  die "cannot reset undefined variable $k" unless defined $bindings;
  ${$$bindings[1]}{$k} = $v;
  $v;
};
...
(def let1 (macro [k v body] ((macro [$k] $body) $v)))
