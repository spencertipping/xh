...
sub xh::corescript::fn::new       {bless [$_[1], $_[2], $_[3]], $_[0]}
sub xh::corescript::native::new   {bless [$_[1], $_[2]], $_[0]}

# TODO: replace this with variant-tracking. Integrals over stuff, maybe.
sub xh::corescript::literal::concrete  {1}
sub xh::corescript::var::concrete      { $_[1]->contains($_[0]) }
...
sub xh::corescript::fn::str {
  my ($self) = @_;
  my ($formals, $body, $concrete) = @$self;
  '(' . join(' ', $concrete ? 'fn' : 'macro',
                  array->new(map literal->new($_), @$formals)->str,
                  $body->str) . ')';
}
...
sub xh::corescript::bindings::contains {
  my ($self, $x) = @_;
  my ($parent, $h) = @$self;
  exists($$h{$x}) || $parent && $parent->contains($x);
}
...
sub xh::corescript::literal::get {
  my ($self, $x) = @_;
  my $i = $x->name;
  die "literal index must be number (got $i instead)" unless $i == $i + 0;
...
sub xh::corescript::list::eval {
  my ($self, $bindings) = @_;
  my ($f, @xs) = @$self;
  my $r = $f->invoke($bindings, @xs);
  return $r if defined $r;

  my $unchanged = 1;
  $unchanged = $_ eq $_->eval($bindings) or last for @$self;
  $unchanged ? $self : list->new(map $_->eval($bindings), @$self);
}
...
sub xh::corescript::fn::eval {
  my ($self, $bindings) = @_;
  my ($args, $body, $concrete) = @$self;
  my $newbody = $body->eval($bindings);
  $body eq $newbody ? $self
                    : fn->new($args, $newbody, $concrete);
}
...
sub xh::corescript::list::invoke {
  ...
  $evaled->invoke($bindings, @xs);
}

sub xh::corescript::fn::invoke {
  my ($self, $bindings, @args) = @_;
  my ($formals, $body, $concrete) = @$self;
  my @eargs = map $_->eval($bindings), @args;
  if ($concrete) {
    $_->concrete($bindings) or return list->new($self, @eargs) for @eargs;
  }
  my %scope;
  if ($$formals[-1] =~ /^&/) {
    my @formals = @$formals;
    my $rest = substr pop(@formals), 1;
    @scope{@$formals} = @eargs[0..$#formals] if @formals;
    $scope{$rest}     = array->new(@eargs[@formals..$#eargs]);
  } else {
    @scope{@$formals} = @eargs;
  }
...
sub deabstractify {
  ...
  native->new($name, sub {
    ...
    $_->concrete($bindings) or return list->new($lname, @ys) for @ys;
    my $result = eval {&$f($bindings, @ys)};
    return undef if $@;
    $result;
  });
}
...
defnglobal apply => sub {
  my ($bindings, $f, @xs) = @_;
  my $ref = pop @xs;
  $f->invoke($bindings, map quote($_), @xs, @$ref);
};

defnglobal array => sub { array->new(@_[1..$#_]) };
...
defnglobal bool => sub { bool $_[1]->name };

defnglobal concrete => sub { bool $_[1]->concrete($_[0]) };
defnglobal contains => sub { bool $_[1]->contains($_[2]) };
defnglobal count    => sub { literal->new(scalar @{$_[1]}) };

defglobal def => sub {
  my ($bindings, $var, $x) = @_;
  $var = $var->eval($bindings);
...
defnglobal empty => sub { ref($_[1])->new };

defglobal fn => sub {
  my ($bindings, $formals, $body) = @_;
  $formals = $formals->eval($bindings);
  ref($_) eq literal or return undef for @$formals;
  fn->new([map $_->name, @$formals],
          $body->eval($bindings),
          1);
};

defnglobal fbody   => sub { ${$_[1]}[1] };
...
defglobal if => sub {
  ...
  return list->new($if, $cond, $then, $else)
         unless $cond->concrete($bindings);
...
defnglobal keys => sub { array->new(map literal->new($_),
                                        sort keys %{$_[1]}) };
...
defnglobal times => sub {
  my ($bindings, $n, $f, @args) = @_;
  my $result = $nil;
  $n = $n->name;
  for (my $i = 0; $i < $n; ++$i) {
    $result = $f->invoke($bindings, literal->new($i), @args);
  }
  $result;
};
...
defnglobal vals => sub { array->new(map ${$_[1]}{$_}, sort keys %{$_[1]}) };
defnglobal var  => sub { var->new($_[1]->name) };

# Imperative data structure functions
defnglobal 'aset!', sub {
  my ($bindings, $xs, $i, $x) = @_;
  $$xs[$i->name] = $x;
  $xs;
};
...
defnglobal 'def!' => sub {
  my ($bindings, $name, $v) = @_;
  my $k = $name->name;
  $bindings = $$bindings[0]
    while defined $bindings and not exists ${$$bindings[1]}{$k};
  return undef unless defined $bindings;
  ${$$bindings[1]}{$k} = $v;
  $v;
};
...
$::xh::compilers{xh} = sub { evaluate $_ for parse $_[1] };
_ \end{perlcode}
...
\lstset{caption={src/corescript.xh},name={src/corescript.xh}}\begin{xhcode}
...
(def map
  (fn [f xs]
    (flatmap (macro [x] [(f $x)]) $xs)))

(def filter
  (fn [f xs]
    (flatmap (macro [x] (if (f $x) [$x] [])) $xs)))
