...
sub xh::corescript::list::eval {
  ...
  my @new;
  push(@new, $_->eval($bindings)), $_ eq $new[-1] or last for @$self;
  $new[-1] eq $$self[-1]
    ? $self
    : list->new(@new, map $_->eval($bindings), @$self[@new..$#$self]);
}
...
sub xh::corescript::array::eval {
  ...
  $new[-1] eq $$self[-1]
    ? $self
    : array->new(@new, map $_->eval($bindings), @$self[@new..$#$self]);
}
...
sub xh::corescript::var::invoke {
  ...
  return quote_call $self, $args if $e eq $self;
...
sub xh::corescript::literal::invoke {
  ...
  return quote_call $self, $args unless defined $f;
...
sub xh::corescript::list::invoke {
  ...
  my $error = $@;
  if ($error) {
    print STDERR '(apply ', $self->str, ' ', $args->str, '): ',
                 $error, "\n";
    die $error;
  } else {
    $result;
...
sub xh::corescript::fn::invoke {
  die 'timeout expired' if $deadline and time > $deadline;
...
defglobal delay => sub {
  my ($bindings, $f, @args) = @_;
  $_->concrete or return undef for @args;
  $f->invoke($bindings, array->new(@args));
};
...
defglobal empty => sub { ref($_[1])->new };
...
defglobal if => sub {
  ...
  return list->new($if, $cond, $then, $else) unless $cond->concrete;
...
defglobal name => sub { literal->new($_[1]->name) };

defglobal print => sub {
  my ($bindings, @args) = @_;
  print STDERR join(' ', map $_->str, @args), "\n";
  $nil;
};
...
defglobal reduce => sub {
  ...
  $init = $f->invoke($bindings, array->new($init, $_)) for @$xs;
  $init;
};
...
defglobal times => sub {
  ...
  $n = $n->name;
  for (my $i = 0; $i < $n; ++$i) {
    $result = $f->invoke($bindings, array->new(literal->new($i), @args));
  }
  $result;
};
...
$::xh::compilers{xh} = sub {
  evaluate($_, $global_bindings, 2) for parse $_[1];
};
_ \end{perlcode}
...
(def fn1
  (fn* args
    (fn* (get $args 0)
      (scope (destructuring-bind! (get $args 1) (var (get $args 0)) {})
        (get $args 2)))))

# BUG: $body evaluating into a list blocks the evaluation of
# (destructuring-bind!), which in turn causes infinite loops. We need to
# work around this by either designating some parameters as being ok to
# bind lazily, or by quoting the list somehow. The former is a superior
# option if it's possible.
(def fn
  (fn1 fn~ [fname fformals body]
    (fn* $fname
      (scope (destructuring-bind! $fformals (var $fname) {})
        $body))))
 \end{xhcode}
