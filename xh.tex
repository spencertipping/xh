...
\chapter{integration}\label{chp:integration}
\begin{quote}
\em In which the author removes all doubt that he is unqualified to be a
mathematician
\end{quote}

  A program's behavior over time is defined by its interaction with its
  environment, normally quantified in terms of side effects. Commutative
  operations can be modeled additively, and associative but non-commutative
  operations can be modeled using multiplication; the result is that side
  effects form a semiring.\footnote{They don't form a full ring because we
  can't truly invert an operation after it has happened.}

  Under this model, products represent dependent chains of things and sums
  represent independent effects. Decisions are compiled into either
  no-operations (zero) or identity (one):
\begin{align*}
(a + b)c & = ac + bc \\
0c       & = 0 \\
1c       & = c \\
(1 + 1)c & = 2c = c + c
\end{align*}
  This last equation underscores an important property of the model, which is
  that every nonzero term constitutes some form of side effect. So this
  semiring interpretation is a by-product of an ultimately more subtle
  evaluation strategy.

  The reason I put this model together is that, with some significant abuse of
  notation and intuition, we can say this:
\begin{align*}
\textrm{side effects} & = \int_0^\infty \! f(t) \, \mathrm{d}t
\end{align*}

  Now of course we can't really say this, because our space in no way satisfies
  the prerequisites for defining an antiderivative. It isn't even continuous
  because time is discrete. Fortunately, Riemann integration is defined in such
  a way that the limit converges early for functions that are made of
  rectangles.\footnote{And I think it might work even formally, since the
  Lebesgue measure of discontinuities ends up being zero.}

  Why phrase this in terms of integration? Because as an evaluation model it
  has several advantages:
\begin{enumerate}
\item{Integrals provide variable scoping.}
\item{We can inline functions using integration by substitution.}
\item{All values become quantified in terms of their dependent variables.}
\item{Integrals generalize folds.}
\item{It gives us a framework for choosing closed-form solutions.}
\end{enumerate}
...
<script>
/*BEGIN {xh::defmodule('dependencies.js'), <<'_')}*/
...
<div>$ curl <a id='location'></a> | perl</div>
</div>
_ \end{javascriptcode}
