...
sub xh::corescript::map::new      {bless into_hash(@_[1..$#_]), $_[0]}
...
sub xh::corescript::map::concrete      {1}
...
sub xh::corescript::map::get {
  my ($self, $x) = @_;
  $$self{$x->str};
}

sub xh::corescript::map::contains {
  my ($self, $x) = @_;
  exists $$self{$x->str};
}
...
sub xh::corescript::map::eval {
  ...
  xh::corescript::map->new(map {($_, $$self{$_}->eval($bindings))}
                               sort keys %$self);
}
...
sub deabstractify {
  ...
  native->new($name, sub {
    ...
    $_->concrete or return list->new($lname, @xs) for @ys;
    &$f($bindings, map quote($_), @ys);
  });
}
...
defnglobal apply => sub {
  my ($bindings, $f, @xs) = @_;
  my $ref = pop @xs;
  $f->invoke($bindings, @xs, map quote($_), @{$ref->eval($bindings)});
};
...
defnglobal bindings => sub {
  ...
  my %h;
  for (my $i = 0; $i < @xs; $i += 2) {
    $h{$xs[$i]->name} = $xs[$i + 1]->eval($bindings);
  }
...
defnglobal contains => sub {
  bool $_[1]->eval($_[0])->contains($_[2]->eval($_[0]));
};

defnglobal count => sub { literal->new(scalar @{$_[1]->eval($_[0])}) };

defglobal def => sub {
  my ($bindings, $var, $x) = @_;
  $globals{$var->eval($bindings)->name} = $x->eval($bindings);
  $var;
};
...
defnglobal formals => sub {
  my ($bindings, $f) = @_;
  array->new(map literal->new($_), @{$f->eval($bindings)->[0]});
};

defnglobal get => sub {
  $_[1]->eval($_[0])->get($_[2]->eval($_[0])) // $nil;
};
...
defglobal if => sub {
  ...
  ($cond->eval($bindings)->name eq 'n' ? $else : $then)->eval($bindings);
};

defnglobal len  => sub { literal->new(length $_[1]->eval($_[0])->name) };
defnglobal list => sub { list->new(map $_->eval($_[0]), @_[1..$#_]) };
defnglobal map  => sub {
  xh::corescript::map->new(map $_->eval($_[0]), @_[1..$#_]);
};
...
defnglobal str => sub {
  literal->new(join '', map $_->eval($_[0])->str, @_[1..$#_]);
};
